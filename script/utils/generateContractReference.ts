...
console.log("Generating solidity docs from `out` artifacts... If the script is failing, and you haven't applied the hack necessary, please see the DEV NOTE in this script.");
...
const main = async () => {
  ...
  // Prize Pool
  {
    const prizePoolSolc = JSON.parse(await fs.readFile("out/PrizePool.sol/PrizePool.json", "utf-8"));
  ...
  // Vault & Vault Factory
  {
    const vaultSolc = JSON.parse(await fs.readFile(join(solcOutDir, "Vault.sol/Vault.json"), "utf-8"));
    const vaultFactorySolc = JSON.parse(await fs.readFile(join(solcOutDir, "VaultFactory.sol/VaultFactory.json"), "utf-8"));
    const iVaultHooksSolc = JSON.parse(await fs.readFile(join(solcOutDir, "IVaultHooks.sol/IVaultHooks.json"), "utf-8"));
  ...
  // CGDA Liquidator
  {
    const liquidationPairSolc = JSON.parse(await fs.readFile(join(solcOutDir, "LiquidationPair.sol/LiquidationPair.json"), "utf-8"));
    const liquidationRouterSolc = JSON.parse(await fs.readFile(join(solcOutDir, "LiquidationRouter.sol/LiquidationRouter.json"), "utf-8"));
    const liquidationPairFactorySolc = JSON.parse(await fs.readFile(join(solcOutDir, "LiquidationPairFactory.sol/LiquidationPairFactory.json"), "utf-8"));
    const continuousGDASolc = JSON.parse(await fs.readFile(join(solcOutDir, "ContinuousGDA.sol/ContinuousGDA.json"), "utf-8"));
    const safeSD59x18Solc = JSON.parse(await fs.readFile(join(solcOutDir, "SafeSD59x18.sol/SafeSD59x18.json"), "utf-8"));
  ...
  {
    const chainlinkVrfV2Direct = JSON.parse(await fs.readFile(join(solcOutDir, "ChainlinkVRFV2Direct.sol/ChainlinkVRFV2Direct.json"), "utf-8"));
    const chainlinkVrfV2DirectRngAuctionHelper = JSON.parse(await fs.readFile(join(solcOutDir, "ChainlinkVRFV2DirectRngAuctionHelper.sol/ChainlinkVRFV2DirectRngAuctionHelper.json"), "utf-8"));
    const iRngAuctionSolc = JSON.parse(await fs.readFile(join(solcOutDir, "IRngAuction.sol/IRngAuction.json"), "utf-8"));
    ...
    await fs.writeFile(
      ...
      `${gitSourceLink("pt-v5-chainlink-vrf-v2-direct", "src/ChainlinkVRFV2DirectRngAuctionHelper.sol")}\n\n`
  ...
  // Claimer
  {
    const claimerSolc = JSON.parse(await fs.readFile(join(solcOutDir, "Claimer.sol/Claimer.json"), "utf-8"));
    const claimerFactorySolc = JSON.parse(await fs.readFile(join(solcOutDir, "ClaimerFactory.sol/ClaimerFactory.json"), "utf-8"));
    const linearVrgdaLibSolc = JSON.parse(await fs.readFile(join(solcOutDir, "LinearVRGDALib.sol/LinearVRGDALib.json"), "utf-8"));
  ...
  // Draw Auction
  {
    ...
    for(const source of sources) {
      builds.push(await solcBuild(JSON.parse(await fs.readFile(join(solcOutDir, source), "utf-8")), solcOutDir));
    }
